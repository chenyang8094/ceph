...
public:
  ...
  void expect_send(MockBootstrapRequest &mock_bootstrap_request,
                   librbd::MockTestImageCtx &mock_local_image_ctx, int r) {
    EXPECT_CALL(mock_bootstrap_request, send())
      .WillOnce(Invoke([&mock_bootstrap_request, &mock_local_image_ctx, r]() {
            if (r == 0) {
...
TEST_F(TestMockImageReplayer, StartStop) {
  ...
  expect_send(mock_bootstrap_request, mock_local_image_ctx, 0);

  EXPECT_CALL(mock_local_journal, add_listener(_));
  expect_is_resync_requested(mock_local_journal, false, 0);
...
TEST_F(TestMockImageReplayer, BootstrapError) {
  ...
  InSequence seq;
  EXPECT_CALL(mock_remote_journaler, construct());
  expect_send(mock_bootstrap_request, mock_local_image_ctx, -EINVAL);
...
TEST_F(TestMockImageReplayer, DelayedReplay) {
  ...
  librbd::journal::EventEntry event_entry(
    librbd::journal::AioDiscardEvent(123, 345), ceph_clock_now(nullptr));
  EXPECT_CALL(mock_local_replay, decode(_, _))
